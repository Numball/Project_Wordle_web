...
import { WORDS } from "./7words.js";//Getting words from external file
...
for (let i = 0; i < NUMBER_OF_GUESSES; i++) {
    ...
    for (let j = 0; j < 7; j++) {
        ...
        box.className = "letter-box"//creating a box element for each letter of guess(7)
...
function insertLetter (pressedKey) {
    if (nextLetter === 7) {//first checking if last letter has been entered
        return
    }
    pressedKey = pressedKey.toLowerCase()

    let row = document.getElementsByClassName("letter-row")[8 - guessesRemaining]//getting the row of the place where letter should be placed
...
function deleteLetter () {
    let row = document.getElementsByClassName("letter-row")[8 - guessesRemaining]//Getting row of deleted letter
...
function checkGuess () {
    ...
    for (let i = 0; i < 7; i++) {//looping through each letter
